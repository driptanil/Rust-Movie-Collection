...
//         pool.execute(
//             "CREATE TABLE IF NOT EXISTS version (id SERIAL PRIMARY KEY, version VARCHAR(255));"
//         ).await.unwrap();
//         pool.execute("INSERT INTO version (version) VALUES ('PostgreSQL 14.2');").await.unwrap();

//         // Create the Actix Web app
//         let app = App::new().app_data(web::Data::new(pool.clone())).configure(service);

//         // Create a test request
//         let request = actix_web::test::TestRequest::get().uri("/version").to_request();
